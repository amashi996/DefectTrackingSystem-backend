...
router.post(
  ...
  [
    ...
    check("defectStatus", "Defect status is required").isIn([
      "New",
      "In Progress",
      "Resolved",
      "Failed",
    ...
    check("defectPriority", "Defect priority is required").isIn([
      "High",
      "Medium",
      "Low",
    ]),
    check("defectSeverity", "Defect severity is required").isIn([
      "Critical",
      "Major",
      "Minor",
      "Cosmetic",
...
// @access  Private
router.put("/updateDefect/:defectId", [auth], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
...
router.put(
  ...
  [auth, check("assignedTo", "AssignedTo user ID is required").notEmpty()],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
...
router.get("/allDefects", auth, async (req, res) => {
  try {
    // Retrieve all defects and populate user name fields
...
router.get("/defect/:defectId", auth, async (req, res) => {
  try {
    // Retrieve the selected defect and populate reportedBy and assignedTo fields
    const defect = await Defect.findById(req.params.defectId);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const attachment = defect.defectAttachment.find(
      ...
      defect.defectAttachment = defect.defectAttachment.filter(
...
// @access  Private
router.get("/viewAttachments/:defectId", auth, async (req, res) => {
  try {
    const defect = await Defect.findById(req.params.defectId);
...
// @access  Private
router.get(
  "/downloadAttachment/:defectId/:attachmentId",
  auth,
  async (req, res) => {
    try {
      const defect = await Defect.findById(req.params.defectId);
...
router.get("/downloadAttachments/:defectId", auth, async (req, res) => {
  try {
    ...
    if (!attachmentIds || attachmentIds.length === 0) {
      return res
        .status(400)
        .json({ msg: "Attachment IDs must be provided as an array" });
    }
    ...
    const selectedAttachments = defect.defectAttachment.filter((attachment) =>
